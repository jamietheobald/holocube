...
for i in [0, 1]:
    s.add_test(apts.on, 1, 0,
               ...
               apts.set_pz, linspace(0, 1.5, numframes), 1,
               apts.set_px, tri * ampl * i, 1,
               bpts.set_pz, linspace(0, 1.5, numframes), 1,
               bpts.set_px, tri * ampl * i, 1,
...
# next move them both together at increasing speeds
spds = [1, 2, 4]
for i in spds:
    s.add_test(apts.on, 1, 0,
               bpts.on, 1, 0,
...
# next move them opposite at varying speeds
spds = [1, 2, 4]
for i in arange(len(spds)):
    aspd = spds[i]
    bspd = spds[-i]
    s.add_test(apts.on, 1, 0,
               bpts.on, 1, 0,
               ...
               apts.set_pz, linspace(0, aspd, numframes), 1,
               apts.set_px, tri * ampl, 1,
               bpts.set_pz, linspace(0, bspd, numframes), 1,
               bpts.set_px, tri * -ampl, 1,
               window.ref_light, lights, 1,
               window.ref_color_4, array(spikelist(expi, numframes) * 255, dtype='int'), 1,
